...
from .const import CONF_MANUFACTURER, CONF_MODEL, CONF_ON_ACTION, DOMAIN, LOGGER, CODES
...
class SamsungTVDevice(MediaPlayerEntity):
    ...
    def command2payloads(self, command):
        ...
        LOGGER.info("Searching for %s", command)
        if command in CODES:
            return [command]
    ...
    async def async_play_media(self, media_type, media_id, **kwargs):
        ...
        for item in media_id.split(','):
            payloads = self.command2payloads(item)
            for key in payloads:
